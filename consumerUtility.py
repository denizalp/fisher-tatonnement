...
def get_SG_utility(allocation, valuations, geary_bias):
    # For ease of calculation, normalize valautions
    # This does not change the preference relation that the utility function represents
    normalized_vals = valuations / np.sum(valuations)
    return np.prod(np.power((allocation - geary_bias), normalized_vals))
...
###### Define UMP and EMP functions for Leontief, StonegGeary CES utility functions ######
# Recall that closed form solutions exist for the UMP anSGEMP problem for both of these function
...
###### Stone-Geary Utilities######

def get_SG_marshallian_demand(prices, budget, valuations, geary_bias):
    normalized_vals = valuations / np.sum(valuations)

    return geary_bias - normalized_vals * budget- (prices.T @ geary_bias)/prices

def get_SG_indirect_util(prices, budget, valuations):
    return get_SG_utility(get_SG_marshallian_demand(prices, budget, valuations), valuations)

def get_SG_ump(prices, budget, valuations):
    """
    Inputs: 
        prices: A vector of non-negative prices
        budget: A non-negative scalar valued budget
    ...
    Returns:
        (indirect_utility, marshallian_demand): A UMP solution pair
            ...
            marshallian_demand: The vector of goods that maximizes utility constrained by budget
    """
    indirect_util = get_SG_indirect_util(prices, budget, valuations)
    marshallian_demand = get_SG_marshallian_demand(prices, budget, valuations)

    return (indirect_util, marshallian_demand)

def get_SG_expend(prices, util, valuations):
    pass
    # normalized_vals = valuations / np.sum(valuations)
    # K = 1/np.prod((np.power(normalized_vals, normalized_vals)))
    # return K*np.prod(np.power(prices, normalized_vals))*util

def get_SG_hicksian_demand(prices, util, valuations):
    pass
    # return get_SG_marshallian_demand(prices, get_SG_expend(prices, util, valuations), valuations)

def get_SG_emp(prices, util_level, valuations):
    """
    Inputs: 
        prices: A vector of non-negative prices
        budget: A non-negative utility level that the buyer wants to at least achieve
    ...
    """
    pass
    # expenditure = get_SG_expend(prices, util_level, valuations)
    # hicksian_demand = get_SG_hicksian_demand(prices, util_level, valuations)
